...
if("gl" IN_LIST FEATURES)
    if (VCPKG_TARGET_IS_LINUX OR VCPKG_TARGET_IS_BSD)
        ...
        string(APPEND OPTIONS " skia_use_egl=true")
    endif()
...
if(VCPKG_TARGET_IS_WINDOWS)
    ...
    # Premation overlay (native/vcpkg-overlays/README.md): build Skia with
    # clang-cl, as Chromium does. Under MSVC Skia falls back to its scalar
    # raster pipeline (no lowp path), which is both several times slower and
    # rounds differently from the Canvas2D the TypeScript rasters come from.
    find_program(PREMATION_CLANG_CL NAMES clang-cl
        HINTS "$ENV{ProgramW6432}/LLVM/bin" "$ENV{ProgramFiles}/LLVM/bin" "C:/Program Files/LLVM/bin"
        NO_DEFAULT_PATH)
    if(PREMATION_CLANG_CL)
        get_filename_component(PREMATION_CLANG_BIN "${PREMATION_CLANG_CL}" DIRECTORY)
        get_filename_component(PREMATION_CLANG_WIN "${PREMATION_CLANG_BIN}" DIRECTORY)
        string(APPEND OPTIONS " clang_win=\"${PREMATION_CLANG_WIN}\"")
        message(STATUS "skia (premation overlay): clang_win=${PREMATION_CLANG_WIN}")
    else()
        message(WARNING "skia (premation overlay): clang-cl not found — building with MSVC (scalar raster pipeline)")
    endif()
