...
struct Record {
  std::uint8_t kind = 0;  // 0 session request, 1 probe
  std::uint32_t step = 0;
  std::string label;
...
Fixture load_fixture() {
  Fixture fx;
  const char* override = std::getenv("D1_FIXTURE");
  std::ifstream f(override != nullptr ? std::string(override) : std::string(PREMATION_ENGINE_TEST_DATA "/d1_eval_parity.bin"), std::ios::binary);
...
Report replay(const SessionData& s, const std::string& portsDir) {
  ...
  for (const Record& r : s.records) {
    rep.records += 1;
    (r.kind == 1 ? rep.probes : rep.requests) += 1;
    ...
    const std::string where = "#" + std::to_string(rep.records - 1) + (r.kind == 1 ? " probe " : " ") + r.label;
    bool differs = false;
    ...
    if (r.hashed) {
      ...
      if (cxErr) {
        ...
        rep.valueDiffs += 1;
        differs = true;
        rep.lines.push_back(where + ": values differ (hashed; D1_FIXTURE=<GEN_NATIVE_D1_FULL file> explains)");
      }
    } else {
...
TEST_CASE("D1: replayed sessions evaluate identically to the TypeScript engine", "[d1][parity]") {
  ...
  const std::filesystem::path dir = std::filesystem::temp_directory_path() / "premation-d1-eval-parity";
  ...
  const bool verbose = std::getenv("D1_VERBOSE") != nullptr;
  std::size_t total = 0;
  ...
  for (const SessionData& s : fx.sessions) {
    ...
    char row[512];
    std::snprintf(row, sizeof row, "  %-60.60s records %4zu probes %4zu | outcome %3zu value %4zu step %3zu (probes %4zu)\n", s.name.c_str(), r.records,
                  r.probes, r.outcomeDiffs, r.valueDiffs, r.stepDiffs, r.probeDiffs);
    table += row;
  ...
  std::printf("[D1 eval parity] %zu sessions (%zu identical), %zu records (%zu probes); differing: %zu (probes %zu)\n%s by request type:\n%s",
              fx.sessions.size(), cleanSessions, totalRecords, totalProbes, total, totalProbeDiffs, table.c_str(), labels.c_str());
  std::filesystem::remove_all(dir);
  CHECK(total <= kMaxMismatches);
}
