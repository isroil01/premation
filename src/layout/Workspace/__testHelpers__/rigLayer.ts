 ...
 * with the box the overlay tests measure against. The fixture writes the
 * geometry and the starting rig directly — setup, not a UI write.
 */
...
export async function rigTestLayer(h: Harness, opts: { width?: number; height?: number; puppet?: unknown; skeleton?: unknown } = {}): Promise<string> {
  ...
  const t = defaultSceneGraph.getNode(layer)!.components.find((c) => c.type === 'Transform')!;
  defaultSceneGraph.writeProp(layer, t.id, 'width', opts.width ?? 200);
  defaultSceneGraph.writeProp(layer, t.id, 'height', opts.height ?? 160);
