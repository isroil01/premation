...
describe('the cost of a bound proxy subtree', () => {
  ...
  it('scales linearly, so the budget above holds for bigger subtrees too', () => {
    // Guards the assumption behind the gate. A super-linear cost would mean the
    // measurement above says nothing about a subtree twice the size.
    ...
    const small = measure(25) + 0.001;
    const large = measure(100);
    // Four times the children should cost roughly four times as much, not
    // sixteen. Generous bound: this is a timing ratio in a hostile realm.
