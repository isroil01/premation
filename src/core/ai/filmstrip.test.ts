...
describe('filmstripTimes', () => {
  it('samples every keyframe, so a short event cannot be stepped over', async () => {
    ...
    const times = await filmstripTimes(ctx, 15);
    expect(times).toContain(6.2);
    expect(times).toContain(7.1);
  });

  it('samples the MIDPOINT of every keyframe pair — that is where easing shows', async () => {
    ...
    expect(await filmstripTimes(ctx, 5)).toContain(2);
  });

  it('always includes the start and the final held frame', async () => {
    const ctx = fakeCtx([{ id: 'a', name: 'A', tracks: [{ prop: 'y', keyframes: [kf(1, 0), kf(2, 1)] }] }]);
    const times = await filmstripTimes(ctx, 8);
  ...
  it('caps the strip', async () => {
    ...
    expect((await filmstripTimes(fakeCtx([{ id: 'a', name: 'A', tracks }]), 12)).length).toBeLessThanOrEqual(STRIP_MAX);
  });

  it('thins from DENSE clusters, keeping isolated events', async () => {
    ...
    const times = await filmstripTimes(fakeCtx([{ id: 'a', name: 'A', tracks: cluster }]), 12);
    expect(times.length).toBeLessThanOrEqual(STRIP_MAX);
    expect(times.some((t) => Math.abs(t - 9.5) < 0.01)).toBe(true);
  });

  it('never samples outside the composition', async () => {
    const ctx = fakeCtx([{ id: 'a', name: 'A', tracks: [{ prop: 'y', keyframes: [kf(-2, 0), kf(99, 1)] }] }]);
    for (const t of await filmstripTimes(ctx, 6)) {
      expect(t).toBeGreaterThanOrEqual(0);
      expect(t).toBeLessThanOrEqual(6);
    }
  });

  it('returns something usable for a scene with no animation at all', async () => {
    expect((await filmstripTimes(fakeCtx([]), 5)).length).toBeGreaterThan(1);
  });
...
describe('sampleVelocities', () => {
  it('measures SPEED, not value — the thing a still cannot show', async () => {
    const ctx = fakeCtx([{ id: 'a', name: 'Mover', tracks: [{ prop: 'x', keyframes: [kf(0, 0), kf(1, 100)] }] }]);
    const [track] = await sampleVelocities(ctx, 2, { samples: 40 });
  ...
  it('ranks by how far a property actually travels', async () => {
    ...
    expect((await sampleVelocities(ctx, 2))[0]!.label).toBe('A.x');
  });

  it('ignores constant tracks and non-hero properties', async () => {
    const ctx = fakeCtx([{
      id: 'a', name: 'A',
      tracks: [
    ...
    expect(await sampleVelocities(ctx, 2)).toEqual([]);
  });

  it('caps the number of graphs', async () => {
    const tracks: FakeTrack[] = ['x', 'y', 'scale', 'scaleX', 'scaleY', 'rotation', 'opacity', 'z']
      .map((prop, i) => ({ prop, keyframes: [kf(0, 0), kf(1, 100 + i)] }));
    expect((await sampleVelocities(fakeCtx([{ id: 'a', name: 'A', tracks }]), 2)).length).toBeLessThanOrEqual(6);
  });
});
