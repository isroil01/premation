...
export class EngineHistoryEntry implements IUndoableCommand {
  /** A History-panel snapshot (`addHistoryCheckpoint`): a named entry that changes nothing. */
  ...
  constructor(engine: LocalEngine, label: string, origin: Origin, before: Parts, after: Parts, named = false) {
    this.engine = engine;
    this.label = label;
    this.origin = origin;
    this.before = before;
...
export class LocalEngine extends EngineClientBase {
  ...
  /** The project's items are the ones its document lists (documentLoad.ts). */
