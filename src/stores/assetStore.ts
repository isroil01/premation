  ...
  immer((set, get) => ({
    ...
    addAsset: (file: File, folderId: string | null = null, opts: AddAssetOptions = {}) =>
      ...
      */
      ...
      if (isLocalFirst()) {
        // Not a document write (NOT_WRITES in scripts/lint/engineWritesRule.mjs): this is the
        // importer itself — it stores the bytes in the bundle blob store. The engine's `importFiles`
        // runs it through its importFile port (appPorts → importMediaFile → addAsset) and adds the
        // item record undoably itself; UI callers of `addAsset` are the sites that move to
        // `importFiles` (a path-less browser File is the known engine gap: no import from bytes).
